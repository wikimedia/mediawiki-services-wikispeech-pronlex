...
class NIZZE {
    ...
    removePrefix: KnockoutObservable<string>;
    removeSuffix: KnockoutObservable<string>;
    
    zmurf: KnockoutComputed<string>;
    
    addPrefixMsg: KnockoutComputed<string>;
    addSuffixMsg: KnockoutComputed<string>;
    removePrefixMsg: KnockoutComputed<string>;
    removeSuffixMsg: KnockoutComputed<string>;
    ...
    constructor() {
        ...
        this.selectedLang = <KnockoutObservable<string>>ko.observable();
	...
	this.removePrefix = <KnockoutObservable<string>>ko.observable("");
        this.removeSuffix = <KnockoutObservable<string>>ko.observable("");
	...
	this.removePrefixMsg = ko.computed({
	    read: () => {
		let itself = this;
		let res = "";

		if (itself.removePrefix() != "" && itself.selectedLang() != "") {
		    
		    let r = new XMLHttpRequest();
		    let url = baseURL + "/decomp/remove_prefix?lang="+ itself.selectedLang() + "&prefix=" + itself.removePrefix();
		    r.open("GET", url);
		    r.onload = function() {
			if ( r.status === 200 ) {

			    console.log("removed prefix: " + r.responseText);
			    res = r.responseText;
			} else {

			    console.error("removed prefix: " + r.responseText);
			    res = r.responseText;
			}
		    };
	...
	this.removeSuffixMsg = ko.computed(
	    {
	    read: () => {
		let itself = this;
		let res = "";
		
		if (itself.removeSuffix() != "" && itself.selectedLang() != "") {
		    
		    let r = new XMLHttpRequest();
		    let url = baseURL + "/decomp/remove_suffix?lang="+ itself.selectedLang() + "&suffix=" + itself.removeSuffix();
		    r.open("GET", url);
		    r.onload = function() {
			if ( r.status === 200 ) {
			    
			    console.log("removed suffix: " + r.responseText);
			    res = r.responseText;
			} else {
			    
			    console.error("removed suffix: " + r.responseText);
			    res = r.responseText;
			}
		    };
