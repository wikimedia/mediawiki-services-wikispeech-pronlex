...
func TestMultipleTags1Sqlite(t *testing.T) {

	dbPath := "./testlex_multipletags.db"
	if _, err := os.Stat(dbPath); !os.IsNotExist(err) {
		err := os.Remove(dbPath)
		if err != nil {
			t.Errorf("failed to remove %s : %v", dbPath, err)
	...
	entries, err := sqliteDBIF{}.lookUpIntoSlice(db, []lex.LexName{lex.LexName(l.name)}, qMulti)
	if err != nil {
	...
	entUpdate, updated, err := sqliteDBIF{}.updateEntry(db, entries[0])
	if err != nil {
		t.Errorf("updateEntry failed : %v", err)
	}
	if !updated {
	...
	// Test Query.MultipleTags
	entries2, err := sqliteDBIF{}.lookUpIntoMap(db, []lex.LexName{lex.LexName(l.name)}, qMulti)
	if err != nil {
		t.Errorf("Got error: %v", err)
	}
